...
final proServiceProvider = Provider((ref) => ProductService(ref.watch(dioProvider)));

final productProvider = FutureProvider((ref) => ProductService(ref.watch(dioProvider)).getData());
...
class ProductService implements ProductInterface{
  ...
  Future<Either<String, bool>> productAdd({
    ...
    try{
      ...
      try {
        ...
        await dio.post(Api.productAdd, data: {
  ...
  Future<Either<String, bool>> productUpdate({
    ...
    try{
      ...
      if(image == null){
        await dio.patch('${Api.productUpdate}/$productId', data: {
          'product_name': product_name,
          'product_detail': product_detail,
          'price': price,
          'photo': 'no need'
  ...
  @override
  Future<Either<String, bool>> productRemove({required String productId, required String imageId}) async {
    try{
      final box = Hive.box<User>('user').values.toList();
        await dio.delete('${Api.productRemove}/$productId', data: {
          'public_id': imageId
        }, options: Options(
          headers: {
            HttpHeaders.authorizationHeader: 'Bearer ${box[0].token}',
